...
// ALL CALCULATIONS IN MILLIMOLES 
  ...
  totalCO2.Resize(NrLayers);              // logs all CO2 generated by oxidation during methane production and transport
...
// With presence of ice the unfrozen water content is taken here to account for smaller diffussivity in the presence of ice
  ...
  {
    dc.Fill(0.0);                          // change in concentration due to plant flux, ebullition, production, oxidation
...
// otherwise the gas reaches the unsaturated zone and is subjected there to oxidation and diffusion
    if (CurrentGW >= (-0.5 * LayerThickness))  // record ebullition flux is top layer is more than 50% saturated, else add it to the first unsaturated layer
    {
      MethaneFlux(1) = MethaneFlux(1) + ebullflux * dth;
    } else if (TopSat > 1) dc(TopSat - 1) = dc(TopSat - 1) + ebullflux * dth; else  dc(1) = dc(1) + ebullflux * dth;
    ccn = cc + dc;
    planttrans (plantflux, CH4oxidation, CO2production, plantrate, ccn); // plant flux
    dc -= plantrate * dth;
    CH4oxidation *= dth;  // CH4 consumed by plant oxidation in time step
    totalCO2 += CO2production * dth; // total CO2 production from oxidation during plant transpor
    plantox += CH4oxidation.Sum(); // Total plant oxidized CH4
    ...
    ccn += dc;
    methaneprod(prod, cres, ccn, CO2production, anaerobCO2, ttime); // methane production including oxidition by methanotrophs above water table
    totprod += prod * dth;  // total CH4 production
    dc = prod * dth; // change due to production
    totalCO2 += CO2production * dth; // add CH4 oxidation in the soil above groundwatertable to total CO2 production
    CH4oxidation = totalCO2; 
    ccn += dc;
...
Input:
c       : total labile C soil in millimol in layer;
pc      : peat C in millimol in layer
mc      : methane concentration millimol in layer

Output:
prod    : total production or oxidation in millimol CH4-C
ttime   : time in days since start of model time step for calculation of rapid saturation depression of methane production
oxidized: matrix with millimoles of CH4-C oxidized per layer
anaerob :output of anarobic CO2 production in millimoles CH4-C
*/
...
//  calculate lag factor for rapid saturation of layers
    ...
    if (t > 0.0)
    {
      ...
      if (anaerobe > 0.0)              // production below groundwater table
      {
        ...
        {
            if (MethaneReservoirs(j) > 0.0)
            {
                 resprod = anaerobe * f * delayfac * MethaneR0Corr(i) * MethaneReservoirs(j) * labileC(i, j);
                 labileC(i, j) -= resprod;
                 pprod += resprod; 
            }
        }
        ...
        anaerob(i) += CO2CH4ratio * pprod;
        pprod = pprod * (1.0 - CO2CH4ratio);
      }
...
// a small fraction of CH4 (<< atmosphere concentration) is assumed to remain to prevent PDE solution problems in cases with deep and rapid falling groundwater tables
      ...
      oxidized(i) = pcons;
      prod(i) = pprod + pcons;                      // total production
    }
  }
}    // end methaneprod
